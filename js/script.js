...
// ================================================================
// ████████  CONTINUAR VIENDO - Módulo completo  ████████
// ================================================================

const CW_KEY = "sf_continue_watching";
const CW_MAX  = 20; // máximo de ítems guardados
...
function cwAddItem(item) {
  ...
  // Si ya existe, eliminarlo para moverlo al frente
  ...
  cwSaveHistory(list);
  cwRenderSection(); // Actualizar la sección si está visible
}

function cwRemoveItem(itemId) {
  const list = cwGetHistory().filter((i) => String(i.id) !== String(itemId));
  cwSaveHistory(list);
  cwRenderSection();
}
...
<section class="content-row cw-section" id="continue-watching-section" style="display:none">
  <div class="row-header">
    <h2 class="section-title">
      <i class="fas fa-play-circle" style="color:var(--secondary-color);margin-right:6px;font-size:1rem;"></i>
      Continuar Viendo
    </h2>
    <button class="cw-clear-btn" id="cw-clear-all" title="Borrar historial">
  ...
  // Inyectar después de la sección En Estreno/Emisión
  const enEstrenoSection = document.querySelector(
    '[aria-labelledby="en-estreno-title"]'
  );
  if (enEstrenoSection) {
    enEstrenoSection.insertAdjacentHTML("afterend", sectionHTML);
  } else {
    // Fallback: inyectar al inicio del main-container
    const mainContainer = document.querySelector(".main-container");
    if (mainContainer) {
      mainContainer.insertAdjacentHTML("afterbegin", sectionHTML);
    }
  }
  ...
  // Activar arrows del slider
  const section = document.getElementById("continue-watching-section");
  if (section) {
    section.querySelectorAll(".slider-prev").forEach((btn) => {
      btn.addEventListener("click", function () {
        const slider = this.closest(".content-slider").querySelector(
          ".slider-container"
        );
        if (slider)
    ...
    section.querySelectorAll(".slider-next").forEach((btn) => {
      btn.addEventListener("click", function () {
        const slider = this.closest(".content-slider").querySelector(
          ".slider-container"
        );
...
function cwRenderSection() {
  ...
  slider.innerHTML = list
    .map((item) => {
      ...
      const stars  = item.vote_average
        ? `⭐ ${Number(item.vote_average).toFixed(1)}`
        : "";
      return `
      <div class="content-card cw-card" data-cw-id="${item.id}">
        <div class="card-poster-wrapper" style="position:relative">
          <img class="card-poster" src="${poster}" alt="${item.title}"
            loading="lazy"
            onerror="this.src='https://via.placeholder.com/150x225?text=Sin+imagen'">
          <button class="cw-remove-btn" data-cw-remove="${item.id}" title="Quitar">
            <i class="fas fa-times"></i>
          </button>
          ${stars ? `<span class="cw-rating">${stars}</span>` : ""}
        </div>
  ...
  slider.querySelectorAll(".cw-remove-btn").forEach((btn) => {
    btn.addEventListener("click", function (e) {
      e.stopPropagation();
      cwRemoveItem(this.dataset.cwRemove);
    });
  });

  // Click en tarjeta → abrir modal del contenido
  slider.querySelectorAll(".cw-card").forEach((card) => {
    card.addEventListener("click", function (e) {
      if (e.target.closest(".cw-remove-btn")) return;
      const cwId = this.dataset.cwId;
      // Buscar el ítem en allContent y abrir su modal
      if (typeof allContent !== "undefined" && allContent.length > 0) {
        const found = allContent.find((c) => String(c.id) === String(cwId));
        if (found && typeof renderModalContent === "function") {
          renderModalContent(found);
        }
      }
    });
  });
...
// ---------- Observar el modal para capturar qué se abre ----------
function cwObserveModal() {
  // Detectar cuándo el modal de detalle se activa con contenido nuevo
  const targetNode =
  ...
  const observer = new MutationObserver(() => {
    // Buscar el título activo del modal para identificar el ítem
    const modalOverlay = document.querySelector(
      ".modal-overlay.active, .modal-overlay[style*='block'], .modal-overlay[style*='flex']"
    );
    if (!modalOverlay) return;

    // Extraer datos del modal abierto desde el DOM
    const titleEl =
      modalOverlay.querySelector(".modal-title, .detail-title, h2.title");
    const posterEl = modalOverlay.querySelector(
      ".detail-poster img, .modal-poster img, img.poster"
    );

    if (!titleEl) return;

    // Buscar en allContent por título para obtener el ítem completo
    if (typeof allContent !== "undefined" && allContent.length > 0) {
      const title = titleEl.textContent.trim();
      const found = allContent.find(
        (c) => (c.title || c.name || "").trim() === title
      );
      if (found) {
        cwAddItem(found);
      }
...
// Esperar a que el DOM y allContent estén listos
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () =>
    setTimeout(initContinueWatching, 800)
  );
} else {
  setTimeout(initContinueWatching, 800);
}
